...
let editing = null, adjusting = null, db, saveQueue = Promise.resolve();
...
const penu = createPenu($, { onChange: () => selectTool(false), rememberedWidth });
function syncPenChoice() { penu.sync(); }
$('penMore').onclick = () => {
...
const selectionHandles = createSelectionHandles(reader, $, { adjusting: () => adjusting });
function handles() { selectionHandles.update(); }
...
reader.addEventListener('selection', ({ detail }) => {
  ...
  $('selection').hidden = reader.selecting && !selectionHandles.dragging;
