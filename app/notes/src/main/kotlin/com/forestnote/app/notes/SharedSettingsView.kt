...
internal class SharedSettingsView(context:Context,private val store:NotebookStore,private val books:ReaderLibraryAccess,
    ...
    enum class Section(val title:Int) { HOME(R.string.settings_title),DEFAULTS(R.string.settings_notebook_defaults),SYNC(R.string.shared_sync_title),RECOGNITION(R.string.recognition_settings_title) }
    ...
    init {
        ...
        show(restored,saved?.getBundle("defaults"))
    }
    ...
    fun back() {
        if(defaults?.saving==true) return
        if(section==Section.HOME) onClose() else show(Section.HOME)
    }
    fun show(next:Section,savedDefaults:Bundle?=null) {
        if(defaults?.saving==true) return
        job?.cancel();job=null;defaults?.close();defaults=null;body.removeAllViews();scroll.scrollTo(0,0)
    ...
    fun snapshot()=Bundle().apply {putString("section",section.name);defaults?.let {putBundle("defaults",it.snapshot())}}
    fun close() {defaults?.close();scope.cancel()}
}
