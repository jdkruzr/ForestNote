...
- Soft-delete (Recycle Bin, AC7): notebook/folder deletes stamp `deleted_at`/`deleted_batch_id`/`deleted_root_id` rather than removing rows. Every query that must ignore tombstones reads the `notebook_live`/`folder_live` VIEWs (not the base table) so the filter can't be forgotten. Standalone notebook delete → NULL batch/root; folder delete → fresh ULID batch shared across the subtree, `deleted_root_id` = the tapped folder. Subtree is computed from LIVE folders BEFORE stamping (they're still live then)
...
- `notebook.sq` - SQLDelight v7 schema (folder/notebook/page/stroke/app_state; `folder` table + `notebook.folder_id`; `notebook.modified_at`; `app_state.settings_json`/`clipboard_json`; `page.template`/`template_pitch_mm`; soft-delete `deleted_at`/`deleted_batch_id`/`deleted_root_id` on folder+notebook; `notebook_live`/`folder_live` VIEWs) and queries (incl. soft-delete mutations `softDeleteNotebook`/`softDeleteNotebooksInFolders`/`softDeleteFolders`; folder CRUD `insertFolder`/`renameFolder`/`getFoldersForParent`/`listAllFolders`/`findFolder` — folder reads go through `folder_live`; Library card reads `listNotebookCardsInFolder`/`listFolderCardsForParent` — correlated subqueries that fold the page/notebook counts in to avoid an N+1; `firstPageIdForNotebook`/`countStrokesForPage` for thumbnails; `getSettingsJson`/`setSettingsJson`, `setPageTemplate`, `bakeNullPageTemplates`, `countPagesForNotebook`, `touchNotebook`)
- `migrations/` - `1.sqm` (v1→v2), `2.sqm` (v2→v3) both DESTRUCTIVE resets; `3.sqm` (v3→v4) adds `notebook.modified_at` (non-destructive, backfilled = created_at); `4.sqm` (v4→v5) adds settings/clipboard + per-page template columns (non-destructive); `5.sqm` (v5→v6) adds the `folder` table + `notebook.folder_id` (non-destructive); `6.sqm` (v6→v7) adds soft-delete columns + indexes AND re-creates the `notebook_live`/`folder_live` views (SQLDelight emits CREATE VIEW only in `create()`, never in `migrate()`, so the migration must do it for upgrading devices)
...
- Schema is at version 7 (auto-derived from the six `.sqm` files); `migrations/2.sqm` is a DESTRUCTIVE v2→v3 reset (drops existing data), consistent with `1.sqm`; `3.sqm` (v3→v4), `4.sqm` (v4→v5), `5.sqm` (v5→v6), and `6.sqm` (v6→v7) are non-destructive
