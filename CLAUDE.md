...
- Storage: SQLDelight 2.0.2 (SQLite). The single library file (`default.forestnote`) lives at **`/sdcard/ForestNote/`** (shared external storage, raw `File` path — no `MANAGE_EXTERNAL_STORAGE`) so it survives an uninstall/reinstall (chiefly the debug→release signing-key swap) and is Termux-inspectable; a private-storage fallback + one-time migration handle the create/migrate cases. Redirected via `StorageLocation`/`ExternalStorageContext` in `core:format` — see `core/format/CLAUDE.md`. Secrets (EncryptedSharedPreferences) stay in private storage.
